...
db.transaction((tx) => {
  ...
  tx.executeSql("DROP TABLE farm_user;");
  //<<<<<<<<<<<<<<<<<<<<<<<< USE ISSO APENAS DURANTE OS TESTES!!! >>>>>>>>>>>>>>>>>>>>>>>

  tx.executeSql(
    "CREATE TABLE IF NOT EXISTS farm_user (id_register INTEGER AUTOINCREMENT,id TEXT PRIMARY KEY UNIQUE, name TEXT, local TEXT, image BLOB, email_user TEXT);"
  );
});
...
const create = (obj) => {
  return new Promise((resolve, reject) => {
    db.transaction((tx) => {
      ...
      tx.executeSql(
        "INSERT INTO farm (id, name, local, email_user) values (?, ?, ?, ?);",
        [obj.id, obj.name, obj.local, obj.email_user],
        //-----------------------
